...
Examples:
    python3 ./train.py -d 0 --identifier baseline config/wireframe.yaml
...
def main():
    ...
    if resume_from:
        ...
        awl.load_state_dict(checkpoint["dynamic"])

    model = DataParallel(model).to(device) # run model parallelly
#    awl = DataParallel(awl).to(device)
    ...
    if C.optim.name == "Adam":
        optim = torch.optim.Adam([
            {'params':model.parameters(), 'lr':C.optim.lr, 'weight_decay':C.optim.weight_decay,'amsgrad':C.optim.amsgrad},
            {'params':awl.parameters(),'weight_decay':0}
        ])
    elif C.optim.name == "SGD":
        optim = torch.optim.SGD([
            {'params':model.parameters(), 'lr':C.optim.lr, 'weight_decay':C.optim.weight_decay,'momentum':C.optim.momentum},
            {'params':awl.parameters(),'weight_decay':0}
        ])
    else:
        raise NotImplementedError
