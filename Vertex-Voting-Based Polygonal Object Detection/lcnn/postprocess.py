...
EXTRACTIONNUM_CENTER=30 # number of centers extracted
EXTRACTIONNUM= 30 # number of corners extracted
EPSILON=10 # hyperparameter for DBSCAN
...
def postprocess(im_info, feature_maps, outdir, NMS=False, plot=False):
    ...
    if plot:
        ...
        for i in centers:
            plt.scatter(i[1], i[0], s=20, color='yellow',zorder=2.5)
...
def center_grouping(cornermap,xoffset,yoffset,centermap,NMS=False):
    ...
    for i in centers:
        result[tuple(i)]=[]
    for i in predictedCenters: # i is predicted center
        tmp=None
        min=float('inf')
        for k in dict[tuple(i)]: # k is corner
            for j in centers: # j is center
                a = dotproduct(subtract(i, k), subtract(j, k))
                b=length(subtract(j,k))
                # vector [corner,predicted center] prejected on vector [corner,center], value can be negative
                projection_length=a/(b+1e-6)
                # filter out the outliers
                if projection_length*1.5>length(subtract(j,k)) and projection_length<1.5*length(subtract(j,k)) and length(subtract(i,j))<min:
                    tmp=j
    ...
    for i in result.keys():
        ...
        # # Do not use when occlusion exists(object centers overlap or close to each other)
